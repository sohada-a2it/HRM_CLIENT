...
import { 
  ...
  Bell, AlertOctagon, Timer, Hourglass, CalendarClock, Watch
} from "lucide-react";
import { Toaster, toast } from "react-hot-toast";

// ===================== AUTO ABSENT FUNCTIONS =====================
const autoAbsentFunctions = {
  // Check and mark auto absent for employees who didn't clock in by 12pm
  checkAndMarkAutoAbsent: async () => {
    try {
      ...
      // Only run if it's between 12:00 PM and 11:59 PM
      if (currentHour < 12) {
        return { checked: false, message: "Too early to check auto absent (before 12pm)" };
      }
      
      const token = getToken();
      if (!token) return { checked: false, message: "No authentication token" };
      
      const today = new Date().toISOString().split('T')[0];
      
      // First, check if today is a working day
      const dayStatus = getDayStatus(today);
      if (dayStatus.status === 'Govt Holiday' || dayStatus.status === 'Weekly Off') {
        return { checked: false, message: `Today is ${dayStatus.status}, no auto absent marking` };
      }
      
      // Fetch all active employees
      const employeesResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/getAll-user`, {
        headers: {
      ...
      if (!employeesResponse.ok) {
        ...
        return { checked: false, message: "Failed to fetch employees" };
      }
      
      const employeesData = await employeesResponse.json();
      const employees = employeesData.users || employeesData || [];
      ...
      for (const employee of employees) {
        ...
        const attendanceResponse = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/records?startDate=${today}&endDate=${today}&employeeId=${employee._id}`,
          {
            headers: {
              "Authorization": `Bearer ${token}`,
              "Content-Type": "application/json"
        ...
        if (attendanceResponse.ok) {
          ...
          const todayAttendance = attendanceData.records?.[0] || attendanceData?.[0];
          
          // If no attendance record exists or record exists but no clock in
          if (!todayAttendance || (!todayAttendance.clockIn && todayAttendance.status !== 'Absent')) {
            
            // Check if employee was on leave
            const isOnLeave = await checkIfEmployeeOnLeave(employee._id, today);
            ...
            if (!isOnLeave) {
              ...
              const absentData = {
                ...
                shiftStart: "09:00",
                shiftEnd: "18:00",
                remarks: `Auto Absent - Didn't clock in by 12pm (${now.toLocaleTimeString()})`,
                autoGenerated: true,
                isAutoAbsent: true,
                location: "System Generated",
                device: "Auto Absent System"
              };
              
              // Create absent record
              const createResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/create-attendance`, {
                method: "POST",
                headers: {
                  "Authorization": `Bearer ${token}`,
                  "Content-Type": "application/json"
              ...
              if (createResponse.ok) {
                autoAbsentCount++;
                console.log(`Auto absent marked for ${employee.firstName} ${employee.lastName}`);
              } else {
                errors.push(`Failed to mark absent for ${employee.firstName} ${employee.lastName}`);
              }
            } else {
              alreadyMarkedCount++;
            }
      ...
      return {
        checked: true,
        message: `Auto absent check completed. Marked ${autoAbsentCount} employees as absent.`,
        details: {
          autoAbsentCount,
          alreadyMarkedCount,
      ...
      console.error("Auto absent check error:", error);
      return { checked: false, message: `Error: ${error.message}` };
    }
  },
  
  // Check if employee is on leave for today
  checkIfEmployeeOnLeave: async (employeeId, date) => {
    try {
      const token = getToken();
      if (!token) return false;
      
      // This should call your leave API
      // For now, we'll assume no leave checking
      return false;
    } catch (error) {
  ...
  // Schedule auto absent check at 12pm daily
  scheduleAutoAbsentCheck: () => {
    const now = new Date();
    const currentHour = now.getHours();
    const currentMinutes = now.getMinutes();
    
    // Calculate time until 12pm
    let hoursUntilNoon = 12 - currentHour;
    let minutesUntilNoon = 0 - currentMinutes;
    ...
    const totalMinutesUntilNoon = (hoursUntilNoon * 60) + minutesUntilNoon;
    const totalMsUntilNoon = totalMinutesUntilNoon * 60 * 1000;
    
    console.log(`Auto absent check scheduled in ${totalMinutesUntilNoon} minutes`);
    
    // Schedule the check
    setTimeout(async () => {
      console.log("Running scheduled auto absent check...");
      const result = await autoAbsentFunctions.checkAndMarkAutoAbsent();
      console.log(result.message);
      
      // Schedule next check for tomorrow
      autoAbsentFunctions.scheduleAutoAbsentCheck();
    }, totalMsUntilNoon);
  }
};
  ...
  generateAttendancePDF: async (attendance, userData, summary) => {
    try {
      ...
      const attendanceData = attendance.map(record => [
        ...
        record.status,
        record.isAutoAbsent ? 'Auto-Absent' : 'Manual'
      ]);
      
      autoTable(doc, {
        startY: detailsY + 5,
        head: [['Date', 'Clock In', 'Clock Out', 'Hours', 'Status', 'Type']],
        body: attendanceData,
        theme: 'grid',
...
// ===================== AUTO ATTENDANCE GENERATION FUNCTIONS =====================

// Auto generate attendance for holidays, weekends, and leave days
const autoGenerateNonWorkingDaysAttendance = async (employeeId, dateString, employeeData) => {
  try {
    const token = getToken();
    ...
    const existingResponse = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/records?startDate=${dateString}&endDate=${dateString}&employeeId=${employeeId}`,
      {
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
    ...
    if (dayStatus.status === 'Govt Holiday') {
      status = 'Govt Holiday';
      remarks = 'Government Holiday - Auto Generated';
    } else if (dayStatus.status === 'Weekly Off') {
      status = 'Weekly Off';
      remarks = 'Weekly Off - Auto Generated';
    } else {
      // It's a working day, no auto generation needed
      return null;
    }
    
    // Prepare attendance data for non-working day
    const attendanceData = {
      employeeId: employeeId,
      date: dateString,
      clockIn: null, // No clock in/out for non-working days
      clockOut: null,
      status: status,
      shiftStart: "09:00",
      shiftEnd: "18:00",
      ...
      holidayType: status === 'Govt Holiday' ? 'Govt Holiday' : null,
      autoGenerated: true,
      location: "System Generated",
      device: "System Auto Generator"
    };
    
    // For admin, use admin endpoint
    let endpoint, method;
    if (isAdmin) {
      endpoint = `${process.env.NEXT_PUBLIC_API_URL}/admin/create-attendance`;
      method = 'POST';
    } else {
      // For employees, they can't auto-generate
      return null;
    }
    ...
    if (response.ok) {
      ...
      console.log(`Auto-generated ${status} attendance for ${employeeData?.firstName} on ${dateString}`);
...
// Auto generate attendance for multiple days
const autoGenerateMultipleDaysAttendance = async (employeeId, startDate, endDate, employeeData) => {
  const results = [];
  ...
  for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
    ...
    // Only generate for non-working days
    if (dayStatus.status === 'Govt Holiday' || dayStatus.status === 'Weekly Off') {
      const result = await autoGenerateNonWorkingDaysAttendance(employeeId, dateString, employeeData);
      if (result) {
        results.push({
          date: dateString,
...
// Check and auto-generate attendance for current month
const checkAndAutoGenerateMonthAttendance = async () => {
  if (!isAdmin) return;
  ...
  try {
    ...
    // For each employee, check and generate for non-working days
    for (const employee of employees) {
      ...
      // Get existing dates
      const existingDates = employeeAttendance.map(a => 
        new Date(a.date).toISOString().split('T')[0]
      );
      
      // Generate for non-working days that don't have attendance
      for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
        const dateString = date.toISOString().split('T')[0];
        const dayStatus = getDayStatus(dateString);
        
        // If it's a non-working day and doesn't have attendance yet
        if ((dayStatus.status === 'Govt Holiday' || dayStatus.status === 'Weekly Off') && 
            !existingDates.includes(dateString)) {
          await autoGenerateNonWorkingDaysAttendance(employee._id, dateString, employee);
    ...
    toast.success("Auto attendance generation completed for non-working days");
  } catch (error) {
    console.error("Auto generate month attendance error:", error);
  }
};
...
const handleDownloadEmployeePDF = async () => {
  ...
  // Handle Auto Absent Trigger (Admin)
  const handleTriggerAutoAbsent = async () => {
    if (!isAdmin) {
      toast.error("Only admin can trigger auto absent");
      return;
    }
    ...
    try {
      ...
      if (result.checked) {
        ...
        if (result.details) {
          // Show details in modal or notification
          toast.success(
            <div>
              <p className="font-bold">Auto Absent Details:</p>
              <p>Marked: {result.details.autoAbsentCount} employees</p>
              <p>Already marked: {result.details.alreadyMarkedCount}</p>
              {result.details.errors && (
                <p className="text-red-500">Errors: {result.details.errors.length}</p>
              )}
            </div>,
            { duration: 6000 }
          );
      ...
      console.error("Trigger auto absent error:", error);
      toast.error("Failed to trigger auto absent");
    } finally {
      setLoading(false);
    }
  };
...
const formatDayMessage = (dateString) => {
  ...
  // New state for filter modal and filtered data
  ...
  // Auto Absent timer state
  const [autoAbsentTimer, setAutoAbsentTimer] = useState(null);
  const [timeUntilAutoAbsent, setTimeUntilAutoAbsent] = useState("");
  const [autoAbsentChecked, setAutoAbsentChecked] = useState(false);
    ...
    const updateAutoAbsentTimer = () => {
      ...
      if (currentHour < 12) {
        ...
        if (hoursLeft === 0 && minutesLeft > 0) {
          setTimeUntilAutoAbsent(`${minutesLeft} minutes until auto absent`);
        } else if (hoursLeft > 0) {
          setTimeUntilAutoAbsent(`${hoursLeft}h ${minutesLeft}m until auto absent`);
        }
      } else if (currentHour === 12 && currentMinutes === 0) {
        setTimeUntilAutoAbsent("Auto absent check in progress...");
      } else {
        setTimeUntilAutoAbsent("Auto absent check completed");
      }
    };
    
    // Update immediately
    updateAutoAbsentTimer();
    
    // Update every minute
    const timer = setInterval(updateAutoAbsentTimer, 60000);
    
    return () => clearInterval(timer);
  }, []);

  // Check for auto absent at 12pm
  useEffect(() => {
    const checkAutoAbsentAtNoon = () => {
      const now = new Date();
      const currentHour = now.getHours();
      const currentMinutes = now.getMinutes();
      
      // Check if it's exactly 12:00 PM
      if (currentHour === 12 && currentMinutes === 0 && !autoAbsentChecked && isAdmin) {
        handleTriggerAutoAbsent();
        setAutoAbsentChecked(true);
        ...
        toast.success(
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold">Auto Absent Check</span>
            </div>
            <p className="text-sm mt-1">Marking employees who didn't clock in by 12pm as absent</p>
          </div>,
          { duration: 5000 }
        );
      }
    ...
    // Check every minute
    const interval = setInterval(checkAutoAbsentAtNoon, 60000);
    
    return () => clearInterval(interval);
  }, [autoAbsentChecked, isAdmin]);
...
const getArrivalStatus = (clockInTime, shiftStartTime = "09:00") => {
  ...
  const fetchAttendanceRecords = useCallback(async (roleInfo) => {
    try {
      ...
      if (response.ok) {
        ...
        // Auto generate attendance for non-working days if admin
        if (roleInfo.isAdmin && roleInfo.isAdmin) {
          // Check for non-working days without attendance
          const startDate = new Date(dateRange.startDate);
          const endDate = new Date(dateRange.endDate);
          
          // Get unique employee IDs from fetched data
          const employeeIds = [...new Set(data.records.map(record => 
            record.employee?._id || record.employee
          ))];
          
          // For each employee, check if non-working days have attendance
          for (const empId of employeeIds) {
            const employee = employees.find(e => e._id === empId);
            if (employee) {
              const employeeAttendance = data.records.filter(record => 
                record.employee?._id === empId || record.employee === empId
              );
              ...
              for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
                ...
                // If it's a non-working day and no attendance exists
                if ((dayStatus.status === 'Govt Holiday' || dayStatus.status === 'Weekly Off') &&
                    !employeeAttendance.some(a => 
                      new Date(a.date).toISOString().split('T')[0] === dateString
                    )) {
                  // Auto generate attendance
                  await autoGenerateNonWorkingDaysAttendance(empId, dateString, employee);
                }
              }
            }
          }
  ...
  }, [dateRange, userId, currentPage, itemsPerPage, employees, isAdmin]);
    ...
    try {
      ...
      if (roleInfo) {
        ...
        if (roleInfo.role === "employee") {
          ...
          // Check if today is a non-working day for employee
          const today = new Date().toISOString().split('T')[0];
          const dayStatus = getDayStatus(today);
          
          if (dayStatus.status === 'Govt Holiday' || dayStatus.status === 'Weekly Off') {
            // Auto generate attendance for today
            await autoGenerateNonWorkingDaysAttendance(
              roleInfo.userData?._id, 
              today, 
              roleInfo.userData
            );
          }
        ...
        // If admin, auto generate for current month
        if (roleInfo.isAdmin) {
          // Small delay then auto generate
          setTimeout(async () => {
            await checkAndAutoGenerateMonthAttendance();
          }, 2000);
          
          // Schedule auto absent check
          autoAbsentFunctions.scheduleAutoAbsentCheck();
        }
      }
    } catch (error) {
  ...
  // Apply employee filter with modal display
  const applyEmployeeFilterWithModal = async () => {
    setLoading(true);
    try {
      const token = getToken();
      ...
      if (response.ok) {
        ...
        // Set filtered records and summary
        ...
        toast.success(`Found ${records.length} records matching your filter!`);
      } else {
  ...
  const handleExportFilteredPDF = async () => {
    ...
    try {
      ...
      if (isAdmin && employeeFilter.employeeId) {
        employeeData = employees.find(emp => emp._id === employeeFilter.employeeId);
      ...
      // Generate PDF
      await pdfUtils.generateAttendancePDF(filteredRecords, employeeData, filterSummary);
  ...
  const closeFilterModal = () => {
    setShowFilterResultsModal(false);
    setFilteredRecords([]);
  ...
  const handleClockIn = async () => {
    ...
    if (!isWorkingDay) {
      ...
      toast.error(`Cannot clock in on a ${dayStatus.status.toLowerCase()}`);
      return;
    }
  ...
  const handleClockOut = async () => {
    ...
    if (!isWorkingDay) {
      ...
      toast.error(`Cannot clock out on a ${dayStatus.status.toLowerCase()}`);
      return;
    }
  ...
  const handleCreateManualAttendance = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const roleInfo = { role: userRole, isAdmin, userData };
        await fetchSummary(roleInfo);
        await applyEmployeeFilterWithModal();
      } else {
  ...
  const getStatusColor = (status) => {
    switch(status?.toLowerCase()) {
      ...
      case 'auto absent': return 'bg-red-100 text-red-800 border-red-200';
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    Showing filtered attendance records
                    {employeeFilter.employeeId && (
                      <span className="ml-2 text-purple-600 font-medium">
                        • Employee: {
                          employees.find(emp => emp._id === employeeFilter.employeeId)?.firstName || 
                          'Selected Employee'
                        }
                      </span>
                    )}
                    {employeeFilter.month && employeeFilter.year && (
                      <span className="ml-2 text-blue-600 font-medium">
                        • {new Date(employeeFilter.year, employeeFilter.month - 1, 1).toLocaleString('default', { month: 'long' })} {employeeFilter.year}
                      </span>
                    )}
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl hover:opacity-90 transition-all duration-300 flex items-center gap-2 disabled:opacity-50"
                  >
                    <Download size={20} />
                    Export PDF
                  </button>
              ...
              {filterSummary && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                  <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-gray-900">{filterSummary.totalDays}</div>
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-green-600">{filterSummary.daysPresent}</div>
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-orange-600">{filterSummary.totalHours.toFixed(2)}</div>
                  </div>
                  ...
                  <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-purple-600">{filterSummary.attendanceRate.toFixed(1)}%</div>
                  </div>
                </div>
              )}
            </div>
              ...
              {filteredRecords.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 max-w-md mx-auto">
                    No attendance records match your filter criteria. Try adjusting your filter options.
                  </p>
                  <button
                    onClick={closeFilterModal}
                    className="mt-6 px-6 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl hover:opacity-90 transition-all duration-300"
                  >
                    Close Modal
                  </button>
                </div>
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center">
                                <div className="flex-shrink-0 h-8 w-8 bg-gradient-to-r from-purple-500 to-pink-600 rounded-full flex items-center justify-center text-white font-bold text-sm">
                                  {record.employee?.firstName?.[0]}{record.employee?.lastName?.[0] || 'E'}
                                </div>
                                <div className="ml-3">
                                  <div className="text-sm font-medium text-gray-900">
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50">
              <div className="flex justify-between items-center">
                <div className="text-sm text-gray-500">
                  ...
                  {filterSummary && ` • ${filterSummary.daysPresent} Present • ${filterSummary.daysAbsent} Absent`}
                </div>
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      closeFilterModal();
                      toast.success("Filter applied to main table!");
                    }}
            ...
            <div className="p-6 border-b border-gray-200 bg-gray-50">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-10 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition-all duration-300"
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3 mt-4 md:mt-0">
              <div className="text-right">
                ...
                {/* Auto Absent Timer Display */}
                {isAdmin && timeUntilAutoAbsent && (
                  <div className={`text-xs mt-2 px-2 py-1 rounded-full font-medium ${
                    timeUntilAutoAbsent.includes("Auto absent check completed") 
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setTimeout(() => applyEmployeeFilterWithModal(), 100);
                    }}
                    ...
                    <div className="text-sm">
                      ...
                      {!todayStatus.clockedIn && (
                        <div className="text-xs text-red-500 mt-2 flex items-center gap-1">
                          ...
                          Clock in before 12pm to avoid auto absent
                        </div>
                      )}
                    </div>
                  </div>
...
<div className="flex gap-2 mt-4 md:mt-0">
  ...
  <button
    onClick={applyEmployeeFilterWithModal}
    disabled={loading}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="p-4 bg-white rounded-xl border border-gray-100 hover:border-gray-200 transition-all duration-300">
                  ...
                  <div className={`inline-flex px-3 py-1 rounded-full text-sm font-semibold ${
                    ...
                    clockDetails.status === 'Auto Absent' ? 'bg-red-100 text-red-800 border border-red-200' :
                    'bg-gray-100 text-gray-800'
                  }`}>
                    {clockDetails.status}
                    {clockDetails.isAutoAbsent && " (Auto)"}
                  </div>
                  {clockDetails.isAutoAbsent && (
                    <div className="text-xs text-red-500 mt-1">
                      Auto marked at 12:00 PM
                    </div>
                  )}
                </div>
              </div>
                          ...
                          <tr key={a._id} className="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(a.status)}`}>
                                {a.status || "Pending"}
                                {(a.status === 'Govt Holiday' || a.status === 'Weekly Off' || a.status === 'Leave') && (
